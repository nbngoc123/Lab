...
def compute_split(m, test_frac=0.15, valid_frac=0.15, min_seasons=3):
    """Chia train/valid/test. Ưu tiên chia theo MÙA (mùa mới nhất=test, mùa kế=valid, các mùa còn lại=train)
    -- đúng tinh thần time-series, dễ hiểu -- nhưng chỉ áp dụng được khi có >= min_seasons mùa (nếu đúng
    2 mùa thì không còn mùa nào rơi vào 'train' -> BUG đã gặp thực tế). Nếu chưa đủ mùa, tự động fallback
    sang chia theo % THỜI GIAN trên toàn bộ trận (đã sort theo match_date) -- luôn ra đủ 3 tập bất kể có
    bao nhiêu mùa. m PHẢI đã sort theo (match_date, match_id) tăng dần (load_matches() đã làm việc này)."""
    seasons_sorted = sorted(m["season"].unique())
    split = pd.Series("train", index=m.index)
    if len(seasons_sorted) >= min_seasons:
        test_season, valid_season = seasons_sorted[-1], seasons_sorted[-2]
        split[m["season"] == valid_season] = "valid"
        split[m["season"] == test_season] = "test"
        print(f"  · split theo MÙA ({len(seasons_sorted)} mùa): train={seasons_sorted[:-2]}, "
        ...
        print(f"  ⚠ chỉ {len(seasons_sorted)} mùa (< {min_seasons}) -> fallback chia theo % THỜI GIAN: "
              f"train={n - n_test - n_valid:,}, valid={n_valid:,}, test={n_test:,} "
              f"(sẽ tự chuyển về chia theo mùa khi backfill đủ {min_seasons} mùa)")
    return split
